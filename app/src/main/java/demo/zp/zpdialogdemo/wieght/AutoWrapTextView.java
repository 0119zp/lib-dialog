...
public class AutoWrapTextView extends View {
    ...
    /**
     * 拆分文本
     * 首先把文本转换成Char数组，然后循环数组把整个文本拆分成N行文本
     */
    private void splitText(int heightMode) {
        ...
        for (int i = 0, length = mTextCharArray.length; i < length; i++) {
            ...
            // 如果currentSingleTextWidth 小于 mSingleTextWidth 就把Char添加到lineStringBuffer 当中，
            // 如果是最后一个Char就直接把lineStringBuffer添加到mSplitTextList集合当中

            // 如果currentSingleTextWidth 大于 mSingleTextWidth，就把lineStringBuffer添加
            // 到mSplitTextList集合当中，重新给lineStringBuffer赋值，currentSingleTextWidth 归0
            ...
            // 循环结束以后拆分好的文本就都添加到mSplitTextList集合当中
        }
